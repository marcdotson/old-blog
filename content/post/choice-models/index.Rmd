...
Stan has considerable promise for making Bayesian inference more accessible in that it provides a high-level interface for using Hamiltonian Monte Carlo (HMC) and HMC-specific diagnostics. In **comparing choice models**, we will be comparing a random-walk Metropolis Hastings Markov chain Monte Carlo (MCMC) implementation of the standard choice model, a hierarchical multinomial logit (HMNL) model, with two different parameterizations of the same model in Stan.

## Model Specification

The complete generative model composed of prior and likelihood may be parameterized in many different ways for the same model. Additionally, as we move beyond the constraints of conjugacy, we can explore a variety of different potential priors.
