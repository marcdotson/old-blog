...
This is what is meant by a hierarchy: there are two levels to our model, the lower-level likelihood and the upper-level population model. Finally, our joint model is complete with a prior on the population model parameters (formally referred to as hyperpriors, since they are priors on a prior): `normal` hyperpriors on `mu` and `tau` (constrained to be positive). Note that we are not assuming a hierarchy to the likelihood variance, now called `sigma`, so it retains the same prior and positive constraint.
...
In this final section, let's fully embrace what it means to have a hierarchical model. With both an observational model *and* a population model, we can include both observation-level and population-level covariates.
...
```
SAMPLING FOR MODEL 'hierarchical_regression_02' NOW (CHAIN 4).
Chain 4: 
Chain 4: Gradient evaluation took 0.000721 seconds
Chain 4: 1000 transitions using 10 leapfrog steps per transition would take 7.21 seconds.
Chain 4: Adjust your expectations accordingly!
Chain 4: 
Chain 4: 
Chain 4: Iteration:    1 / 2000 [  0%]  (Warmup)
...
Chain 4: Iteration: 2000 / 2000 [100%]  (Sampling)
Chain 4: 
Chain 4:  Elapsed Time: 151.058 seconds (Warm-up)
Chain 4:                146.161 seconds (Sampling)
Chain 4:                297.219 seconds (Total)
```

Note that even though we've preemptively set `adapt_delta = 0.99` given the previous model's performance, it will be easy to run into problems given the additional dimensions we've added to the posterior. We'll touch on possible solutions at the end. For now, the model has completed without any warnings.

Let's evaluate trace plots.

```{r trace-plots-02a, eval=FALSE}
...
![](Figures/mcmc_trace-02a.png){width=1000px}

```{r trace-plots-02b, eval=FALSE}
...
![](Figures/mcmc_trace-02b.png){width=1000px}

Once again, we have good mixing and clear convergence across all chains, now for our many model parameters.
...
![](Figures/marginals-02c.png){width=1000px}
...
While we haven't touched on a complete Bayesian inference workflow, including evaluating prior and likelihood specifications using prior and posterior predictive checks, hopefully this provides a helpful starting point for using hierarchical models in Stan. While our model didn't require it, the [posterior geometry of hierarchical models typically require the non-centered parameterization](https://arxiv.org/abs/1312.0906).

Hierarchical models provide partial pooling, a middle-ground between no information pooling (i.e., separate non-hierarchical models for each group) and complete information pooling (i.e., a single non-hierarchical model). They allow for covariates and parameter estimation at both the observation and population levels. Most importantly, they allow for modeling heterogeneity -- differences across groups. Given the prevalence of heterogeneity, hierarchical models should be our default approach in most applications and Stan removes the barriers to entry for the required Bayesian computation.
